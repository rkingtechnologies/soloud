...
class Bus : public AudioSource {
 public:
  ...
  handle play(AudioSource& aSound, float aVolume = 1.0f, float aPan = 0.0f,
    bool aPaused = 0);
  // Play sound through the bus, delayed in relation to other sounds called via
  // this function.
  handle playClocked(time aSoundTime, AudioSource& aSound, float aVolume = 1.0f,
    float aPan = 0.0f);
  ...
  handle play3d(AudioSource& aSound, float aPosX, float aPosY, float aPosZ,
    ...
    float aVolume = 1.0f, bool aPaused = 0);
  // Start playing a 3d audio source through the bus, delayed in relation to
  // other sounds called via this function.
  handle play3dClocked(time aSoundTime, AudioSource& aSound, float aPosX,
    ...
    float aVelZ = 0.0f, float aVolume = 1.0f);
  ...
  // Get approximate volume for output channel for visualization. Visualization
  // has to be enabled before use.
