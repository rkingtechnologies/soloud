...
class Pool {
 public:
  // Initialize and run thread pool. For thread count 0, work is done at addWork
  // call.
  ...
  ~Pool();
  // Add work to work list. Object is not automatically deleted when work is
  // done.
  void addWork(PoolTask* aTask);
  // Called from worker thread to get a new task. Returns null if no work
  // available.
  PoolTask* getWork();
