...
class AudioCollider {
 public:
  ...
  virtual float collide(Soloud* aSoloud,
    AudioSourceInstance3dData* aAudioInstance3dData, int aUserData) = 0;
};

class AudioAttenuator {
 public:
  virtual float attenuate(float aDistance, float aMinDistance,
    float aMaxDistance, float aRolloffFactor) = 0;
};
...
class AudioSourceInstance {
 public:
  enum FLAGS {
    ...
    LOOPING = 1,
    // This audio instance is protected - won't get stopped if we run out of
    // voices
    PROTECTED = 2,
  ...
  virtual unsigned int getAudio(
    float* aBuffer, unsigned int aSamplesToRead, unsigned int aBufferSize) = 0;
  // Has the stream ended?
  virtual bool hasEnded() = 0;
  // Seek to certain place in the stream. Base implementation is generic "tape"
  // seek (and slow).
  virtual result seek(
    time aSeconds, float* mScratch, unsigned int mScratchSize);
  // Rewind stream. Base implementation returns NOT_IMPLEMENTED, meaning it
  // can't rewind.
...
class AudioSource {
 public:
  enum FLAGS {
    ...
    PROCESS_3D = 8,
    // Audio instances created from this source have listener-relative 3d
    // coordinates
    LISTENER_RELATIVE = 16,
  ...
  // Set whether only one instance of this sound should ever be playing at the
  // same time
  ...
  // Set the minimum and maximum distances for 3d audio source (closer to min
  // distance = max vol)
  ...
  // Set the coordinates for this audio source to be relative to listener's
  // coordinates.
