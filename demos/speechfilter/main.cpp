...
void RenderSpeechfilterUI() {
  ...
  ImGui::Text("Music volume     : %d%%",
    (int)floor(gSoloud.getVolume(gSpeechhandle) * 100));
  ImGui::Text("Active voices    : %d", gSoloud.getActiveVoiceCount());

  ImGui::SeparatorText("Filters");
  if (ImGui::CollapsingHeader("WaveShaper")) {
    ImGui::SliderFloat("Wet##5", &filter_param0[4], 0, 1);
    ImGui::SliderFloat("Amount##5", &filter_param1[4], -1, 1);
  }
  if (ImGui::CollapsingHeader("Biquad (lowpass)")) {
  ...
  if (ImGui::CollapsingHeader("LoFi")) {
  ...
  if (ImGui::CollapsingHeader("DC removal")) {
    ImGui::SliderFloat("Wet##1", &filter_param0[3], 0, 1);
  }
  if (ImGui::CollapsingHeader("Robotize")) {
    ImGui::SliderFloat("Wet##5", &filter_param0[5], 0, 1);
  }

  ImGui::SeparatorText("Speech params");
  int changed = 0;
  if (ImGui::CollapsingHeader("Engine")) {
    if (ImGui::RadioButton(
          "Speech", gSoloud.getPause(gSpeechhandle) == false)) {
  ...
  if (ImGui::CollapsingHeader("Base params")) {
    if (ImGui::SliderFloat("Base freq", &basefreq, 0, 3000)) {
      changed = 1;
    }
    if (ImGui::SliderFloat("Base speed", &basespeed, 0.1f, 30)) {
      changed = 1;
    }
    if (ImGui::SliderFloat("Base declination", &basedeclination, -3, 3)) {
      changed = 1;
    }
  }
  if (changed) {
    gSpeech.setParams(
      (unsigned int)floor(basefreq), basespeed, basedeclination, basewaveform);
  }
}

void InitSpeech() {
  gVizsn.setText(
    "The beige hue on the waters of the loch impressed all, including the "
    ...
    "headgear? "
    "Shaw, those twelve beige hooks are joined if I patch a young, gooey "
    "mouth. "
    "With tenure, Suzie'd have all the more leisure for yachting, but her "
    "publications are no good.");
  gSpeech.setText(
    "The beige hue on the waters of the loch impressed all, including the "
    "French queen, before she heard that symphony again, just as young Arthur "
    "wanted. "
...
void InitAudio() {
  // Initialize SoLoud with visualization support
