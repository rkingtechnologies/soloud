...
void RenderMonotone() {
  ...
  ImGui::Text("Music volume     : %d%%",
    (int)floor(gSoloud.getVolume(gMusichandle) * 100));
  ...
  if (ImGui::CollapsingHeader("Waveform", &collapsed)) {
    ...
    if (ImGui::RadioButton(
          "Fourier square", waveform == SoLoud::Soloud::WAVE_FSQUARE)) {
      waveform = SoLoud::Soloud::WAVE_FSQUARE;
      gMusic.setParams(hwchannels, waveform);
    }
    if (ImGui::RadioButton(
          "Fourier saw", waveform == SoLoud::Soloud::WAVE_FSAW)) {
