...
const char* phrase[19] = {"............................", "in empty eternity",
  "rain pours down from above", "Because it is", "but that was then",
  "Let everyone know", "in the spaces between", "than your bitter heart",
  "a cry of seabirds high over", "in my head", "sit down and please don't move",
  "I don't feel good", "Even if time", "For all it's worth.", "the only sound",
  "a vast thrumming of crickets", "voices of angels", "don't bother me",
  "but from an early age"};

void RenderRadioGaga() {
  int tick = ImGui::GetTime() * 1000;
  ...
  if (gSoloud.getPause(gSpeechqueuehandle) &&
      !gSoloud.isValidVoiceHandle(gIntrohandle)) {
...
int main() {
  ...
  SimpleWindow simple_window(RenderRadioGaga);
  soloud::tests::common::Window window({"Radiogaga Demo", 330, 600});
