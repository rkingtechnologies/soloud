...
SNDBUFFER::SNDBUFFER(unsigned aSize) {
  read_position = 0;
  samples_ready = 0;
  // size must be power of 2 for faster calc cyclic position: pos = (pos+1) &
  // (size-1)
  if (aSize & (aSize - 1)) {
    unsigned i = 1;
    while (i < aSize) {
...
void SNDBUFFER::get_buffers(bufptr_t& pos1, unsigned& size1, bufptr_t& pos2,
