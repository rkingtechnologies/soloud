...
const unsigned SNDR_DEFAULT_AY_RATE =
...
class SNDCHIP : public SNDRENDER {
 public:
  ...
  static const char* get_chipname(CHIP_TYPE i) {
    return ay_chips[i];
  }

  void set_chip(CHIP_TYPE type) {
    chiptype = type;
  }
  void set_timings(
    unsigned system_clock_rate, unsigned chip_clock_rate, unsigned sample_rate);
  void set_volumes(unsigned global_vol, const SNDCHIP_VOLTAB* voltab,
    const SNDCHIP_PANTAB* stereo);
  void set_volume(unsigned global_vol);

  void reset(
    unsigned timestamp = 0);  // call with default parameter, when context
                              // outside start_frame/end_frame block

  // 'render' is a function that converts array of register writes into
  // PCM-buffer
  ...
  unsigned char get_activereg() {
    return activereg;
  }
  unsigned char get_r13_reloaded() {
    return r13_reloaded;
  }
  unsigned char get_reg(unsigned nreg) {
    return reg[nreg];
  }
  unsigned get_env() {
    return env;
