...
ChipPlayer::ChipPlayer(SoLoud::AyInstance* aInstance)
  ...
  // set_timings(SNDR_DEFAULT_SYSTICK_RATE * 40 / 17, SNDR_DEFAULT_AY_RATE * 20
  // / 17, SNDR_DEFAULT_SAMPLE_RATE);
  set_timings(aInstance->mParent->mCpuspeed, aInstance->mParent->mChipspeed,
    SNDR_DEFAULT_SAMPLE_RATE);
...
void ChipPlayer::set_timings(
  unsigned aSystem_clock_rate, unsigned chip_clock_rate, unsigned sample_rate) {
  ChipPlayer::system_clock_rate = aSystem_clock_rate;
  ticks_per_buffer =
    (unsigned)((long long)system_clock_rate * (buffer.size - 20) / sample_rate);
...
const int VOL_DIV = 2;  // mixer just adds samples. to avoid overflow, we lower
                        // volume then revert back with saturation

void ChipPlayer::set_volumes(SNDCHIP::CHIP_TYPE t, unsigned global_vol,
...
unsigned ChipPlayer::play(float* dst, unsigned need_samples) {
  ...
  while (need_samples) {
    ...
    for (unsigned i = 0; i < from_buffer; i++) {
      ...
      SNDSAMPLE& s =
        buffer.buffer[(buffer.read_position + i) & (buffer.size - 1)];
      //*dst++ = s;
    ...
    while ((unsigned)(playtick - frame_tick) < ticks_per_buffer) {
      ...
      do {
        ...
        (rd & 0x8000) && !(mInstance->mPos >= mInstance->mParent->mLength));
