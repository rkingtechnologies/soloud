...
class SNDBUFFER {
 public:
  ...
  void get_buffers(bufptr_t& pos1, unsigned& size1, bufptr_t& pos2,
    unsigned& size2, unsigned count);

  // can read some samples starting from here, use count_start(), count(),
  // count_end() to detect count of samples ready
