...
void FFTFilterInstance::filterChannel(float* aBuffer, unsigned int aSamples,
  float aSamplerate, double aTime, unsigned int aChannel,
  ...
  // Could allocate max_channels but that would potentially waste a lot of
  // memory.
  if (mInputBuffer == 0) {
  ...
  while (ofs < aSamples) {
    ...
    for (i = 0; i < samples; i++) {
      mInputBuffer[chofs + ((inputofs + STFT_WINDOW_HALF) &
                             (STFT_WINDOW_TWICE - 1))] = aBuffer[ofs + i];
      mMixBuffer[chofs +
                 ((inputofs + STFT_WINDOW_HALF) & (STFT_WINDOW_TWICE - 1))] = 0;
      inputofs++;
    }
    ...
    if ((inputofs & (STFT_WINDOW_HALF - 1)) == 0) {
      for (i = 0; i < STFT_WINDOW_SIZE; i++) {
        mTemp[i] =
          mInputBuffer[chofs +
                       ((inputofs + STFT_WINDOW_TWICE - STFT_WINDOW_HALF + i) &
                         (STFT_WINDOW_TWICE - 1))];
      }
      ...
      // do magic
      fftFilterChannel(
        mTemp, STFT_WINDOW_HALF, aSamplerate, aTime, aChannel, aChannels);
    ...
    for (i = 0; i < samples; i++) {
      aBuffer[ofs + i] +=
        (mMixBuffer[chofs + (readofs & (STFT_WINDOW_TWICE - 1))] -
          aBuffer[ofs + i]) *
        mParam[0];
      readofs++;
    }
...
void FFTFilterInstance::comp2MagPhase(
  float* aFFTBuffer, unsigned int aSamples) {
...
void FFTFilterInstance::magPhase2MagFreq(float* aFFTBuffer,
  unsigned int aSamples, float aSamplerate, unsigned int aChannel) {
...
void FFTFilterInstance::magPhase2Comp(
  float* aFFTBuffer, unsigned int aSamples) {
  for (unsigned int i = 0; i < aSamples; i++) {
    float mag = aFFTBuffer[i * 2];
    float pha = aFFTBuffer[i * 2 + 1];
...
void FFTFilterInstance::fftFilterChannel(float* aFFTBuffer,
  unsigned int aSamples, float aSamplerate, time /*aTime*/,
