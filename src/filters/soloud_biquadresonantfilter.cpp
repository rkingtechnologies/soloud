...
void BiquadResonantFilterInstance::filterChannel(float* aBuffer,
  unsigned int aSamples, float aSamplerate, double aTime, unsigned int aChannel,
  ...
  for (i = 0; i < aSamples; i += 2, c++) {
    ...
    x = aBuffer[c];
    s.mY2 =
      (mA0 * x) + (mA1 * s.mX1) + (mA2 * s.mX2) - (mB1 * s.mY1) - (mB2 * s.mY2);
    ...
    s.mX2 = aBuffer[c];
    s.mY1 =
      (mA0 * s.mX2) + (mA1 * x) + (mA2 * s.mX1) - (mB1 * s.mY2) - (mB2 * s.mY1);
...
result BiquadResonantFilter::setParams(
  int aType, float aFrequency, float aResonance) {
