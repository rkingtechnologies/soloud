...
void DuckFilterInstance::filter(float* aBuffer, unsigned int aSamples,
  unsigned int aBufferSize, unsigned int aChannels, float aSamplerate,
  double aTime) {
  updateParams(aTime);
  float onramp_step = 1;
  if (mParam[DuckFilter::ONRAMP] > 0.01) {
    onramp_step = (1.0f - mParam[DuckFilter::LEVEL]) /
                  (mParam[DuckFilter::ONRAMP] * aSamplerate);
  }
  float offramp_step = 1;
  if (mParam[DuckFilter::OFFRAMP] > 0.01) {
    offramp_step = (1.0f - mParam[DuckFilter::LEVEL]) /
                   (mParam[DuckFilter::OFFRAMP] * aSamplerate);
  }
  ...
  for (unsigned int j = 0; j < aChannels; j++) {
    ...
    for (unsigned int i = 0; i < aSamples; i++) {
      ...
      aBuffer[i + bchofs] +=
        (-aBuffer[i + bchofs] + aBuffer[i + bchofs] * level) *
        mParam[DuckFilter::WET];
    }
  }
  mCurrentLevel = level;
}
...
result DuckFilter::setParams(Soloud* aSoloud, handle aListenTo, float aOnRamp,
  float aOffRamp, float aLevel) {
