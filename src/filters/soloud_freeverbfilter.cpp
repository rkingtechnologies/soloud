...
// Based on code written by Jezar at Dreampoint, June 2000
// http://www.dreampoint.co.uk, which was placed in public domain. The code was
// massaged quite a bit by Jari Komppa, result in the license listed at top of
// this file.
...
void FreeverbFilterInstance::filter(float* aBuffer, unsigned int aSamples,
...
result FreeverbFilter::setParams(
  float aFreeze, float aRoomSize, float aDamp, float aWidth) {
  if (aFreeze < 0 || aFreeze > 1 || aRoomSize <= 0 || aDamp < 0 ||
      aWidth <= 0) {
