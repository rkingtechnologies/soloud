...
unsigned int BusInstance::getAudio(
  ...
  s->mixBus_internal(aBuffer, aSamplesToRead, aBufferSize, mScratch.mData,
    handle, mSamplerate, mChannels, mParent->mResampler);
...
void Bus::findBusHandle() {
  if (mChannelHandle == 0) {
    ...
    int i;
    for (i = 0; mChannelHandle == 0 && i < (signed)mSoloud->mHighestVoice;
      i++) {
...
handle Bus::playClocked(
  time aSoundTime, AudioSource& aSound, float aVolume, float aPan) {
  if (!mInstance || !mSoloud) {
    return 0;
  }
  ...
  return mSoloud->playClocked(
    aSoundTime, aSound, aVolume, aPan, mChannelHandle);
}

handle Bus::play3d(AudioSource& aSound, float aPosX, float aPosY, float aPosZ,
  float aVelX, float aVelY, float aVelZ, float aVolume, bool aPaused) {
  if (!mInstance || !mSoloud) {
    return 0;
  }
  ...
  return mSoloud->play3d(aSound, aPosX, aPosY, aPosZ, aVelX, aVelY, aVelZ,
    aVolume, aPaused, mChannelHandle);
}

handle Bus::play3dClocked(time aSoundTime, AudioSource& aSound, float aPosX,
  ...
  return mSoloud->play3dClocked(aSoundTime, aSound, aPosX, aPosY, aPosZ, aVelX,
    aVelY, aVelZ, aVolume, mChannelHandle);
}
...
unsigned int Bus::getActiveVoiceCount() {
  ...
  for (i = 0; i < VOICE_COUNT; i++) {
    if (mSoloud->mVoice[i] &&
        mSoloud->mVoice[i]->mBusHandle == mChannelHandle) {
      count++;
    }
  }
  mSoloud->unlockAudioMutex_internal();
