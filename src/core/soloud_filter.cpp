...
void FilterInstance::fadeFilterParameter(
  unsigned int aAttributeId, float aTo, double aTime, double aStartTime) {
...
void FilterInstance::oscillateFilterParameter(unsigned int aAttributeId,
  float aFrom, float aTo, double aTime, double aStartTime) {
...
void FilterInstance::filter(float* aBuffer, unsigned int aSamples,
  unsigned int aBufferSize, unsigned int aChannels, float aSamplerate,
  double aTime) {
  unsigned int i;
  for (i = 0; i < aChannels; i++) {
    filterChannel(
      aBuffer + i * aBufferSize, aSamples, aSamplerate, aTime, i, aChannels);
  }
}

void FilterInstance::filterChannel(float* /*aBuffer*/,
  unsigned int /*aSamples*/, float /*aSamplerate*/, double /*aTime*/,
